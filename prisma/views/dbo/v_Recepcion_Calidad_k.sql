SELECT
  ...
  (
    SELECT
      SUM(CANT_ENV_CONT) AS Expr1
    FROM
      Erpfrusys.dbo.RECEPACK_DET AS rd
    WHERE
      (COD_TEM = '7')
      AND (COD_EMP = 'MER')
      AND (PLANILLA = R.PLANILLA)
  ) AS totes_sp
FROM
  Erpfrusys.dbo.RECEPACK AS R
  JOIN Erpfrusys.dbo.PRODUCTORES AS P ON R.COD_EMP = P.COD_EMP
  AND R.COD_TEM = P.COD_TEM
  AND R.COD_PRO = P.COD_PRO
  JOIN Erpfrusys.dbo.CTRL_TIT_DATOS AS CTD ON R.COD_EMP = CTD.COD_EMP
  AND R.COD_TEM = CTD.COD_TEM
  AND R.ZON = CTD.ZON
  AND R.PLANILLA = CTD.PLANILLA_REC
  AND R.LOTE = CTD.LOTE
  JOIN Erpfrusys.dbo.CTRL_DATOS AS CD ON CTD.COD_EMP = CD.COD_EMP
  AND CTD.COD_TEM = CD.COD_TEM
  AND CTD.ZON = CD.ZON
  AND CTD.TIPO = CD.TIPO
  AND CTD.PLANILLA = CD.PLANILLA
  AND CTD.PLANILLA_REC = CD.PLANILLA_REC
  JOIN Erpfrusys.dbo.CTRL_ITEM AS CI ON CI.COD_EMP = CD.COD_EMP
  AND CI.COD_TEM = CD.COD_TEM
  AND CI.TIPO = CD.TIPO
  AND CI.COD_ITEM = CD.COD_ITEM
  AND CI.COD_FAMILIA = CD.COD_FAMILIA
  JOIN Erpfrusys.dbo.TIT_RECEPACK AS tr ON tr.COD_EMP = R.COD_EMP
  AND tr.COD_TEM = R.COD_TEM
  AND R.PLANILLA = tr.PLANILLA
  JOIN Erpfrusys.dbo.ESPECIE AS esp ON esp.COD_EMP = R.COD_EMP
  AND esp.COD_TEM = R.COD_TEM
  AND R.COD_ESP = esp.COD_ESP
  JOIN Erpfrusys.dbo.VARIEDAD AS [var] ON [var].COD_EMP = R.COD_EMP
