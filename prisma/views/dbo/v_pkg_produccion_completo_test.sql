SELECT
  ...
  CASE
    WHEN d.tipo = 'Salidas de Proceso'
    OR d.tipo = 'Salidas de Proceso GR' THEN (
      SELECT
        CONVERT(float, isnull(d.kilos, 1)) / (
          SELECT
            SUM(isnull(d2.KILOS, 1)) AS Expr1
          FROM
            dbo.v_produccion_detallado AS d2
          WHERE
            (d2.PLANILLA = d.PLANILLA)
            AND (d2.tipo = 'Ingreso a Proceso')
            AND (d2.COD_TEM = d.COD_TEM)
        )
    )
    ELSE 0
  END AS factor_exp
