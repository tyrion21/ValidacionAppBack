SELECT
  ...
  e.COD_PRO,
  e.COD_FRI,
  ISNULL(e.COD_ESP, p.COD_ESP) AS cod_esp,
  (
    SELECT
      NOM_ESP
    FROM
    ...
    WHERE
      ...
      AND (COD_ESP = ISNULL(e.COD_ESP, p.COD_ESP))
  ) AS n_Especie,
  e.COD_VAR,
  (
    SELECT
  ...
  CASE
    ...
    ELSE 'Error'
  END AS prod_sdp_eti,
  ISNULL(m.PROCESO, ISNULL(m.PK_ORIGINAL, e.PK_ORIGINAL)) AS Expr2
FROM
  ...
  LEFT JOIN Erpfrusys.dbo.PROPACKLOTE AS p ON p.LOTE = e.LOTE
WHERE
  (e.COD_EMP = 'MER')
  AND (e.COD_TEM = '6');